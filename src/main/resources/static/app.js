...
const pages = {
  ...
  contas: ['Contas', 'Pagamentos gerados para pacientes sem plano.'],
...
async function loadAll() {
  ...
  else setLoginHelp('Dados carregados. Selecione um usuário para entrar.');
  updateVisibility();
}
...
function renderAll() {
  renderSession(); renderDashboard(); renderPacientes(); renderMedicos(); renderEnfermeiros(); renderConsultas(); renderTriagens(); renderAvaliacoes(); renderContas();
...
function renderSession() {
  ...
  setText('#session-detail', session?.role === 'MEDICO' ? 'Perfil médico' : session?.role === 'PACIENTE' ? 'Perfil paciente' : 'Demonstração livre');
}

function renderDashboard() {
  setText('#metric-pacientes', state.pacientes.length); setText('#metric-medicos', state.medicos.length); setText('#metric-consultas', state.consultas.length); setText('#metric-triagens', state.triagens.length); setText('#metric-fila', state.fila.length);
  const statuses = { AGENDADA: 0, REALIZADA: 0, CANCELADA: 0, LISTA_ESPERA: state.fila.length };
  state.consultas.forEach(c => { const key = c.status || 'SEM_STATUS'; statuses[key] = (statuses[key] || 0) + 1; });
...
function renderMedicos() {
  ...
  const doctors = state.medicos.filter(m => (!term || normalize(m.nome).includes(term)) && (!specialty || String(m.especialidade) === specialty));
  $('#doctor-search-list').innerHTML = doctors.length ? doctors.map(m => {
    const avg = medicoMedia(m.id); const reviews = ultimasAvaliacoes(m.id, 2);
    return `<article class="doctor-card searchable"><header><div><strong>${escapeHtml(m.nome)}</strong><small>CRM: ${escapeHtml(m.crm || '-')}</small></div>${badge(formatEspecialidade(m.especialidade), 'info')}</header><div class="rating">${stars(avg)} ${avg ? Number(avg).toFixed(1) : 'sem média'}</div><small>Limite diário: ${limiteMedico(m)} paciente(s)</small><div class="mini-list">${reviews.length ? reviews.map(r => `<span>“${escapeHtml(r.texto || 'Sem comentário')}” • ${r.estrelas}★</span>`).join('') : '<span>Sem avaliações recentes</span>'}</div></article>`;
...
function renderContas() {
  ...
  $('#accounts-list').innerHTML = state.contas.length ? state.contas.map(c => `<article class="list-item searchable"><header><div><strong>${escapeHtml(c.pacienteNome || 'Paciente')}</strong><small>Situação: ${escapeHtml(c.situacao || 'PENDENTE')}</small></div>${badge(money(c.valor), 'warn')}</header></article>`).join('') : emptyState('Nenhuma conta pendente.');
}

function fillSelects() {
  fillSelect('#login-user', $('#login-role')?.value === 'MEDICO' ? state.medicos : state.pacientes, 'Selecione um usuário', u => `${u.nome} ${u.crm ? '— ' + formatEspecialidade(u.especialidade) : '— ' + planoLabel(u)}`);
  ...
  fillSelect('#screening-nurse', state.enfermeiros, 'Selecione um enfermeiro', e => `${e.nome} — ${e.coren || '-'}`);
...
function setupForms() {
  $('#login-role')?.addEventListener('change', fillSelects);
  bindForm('#login-form', async form => {
    const data = formData(form);
    state.session = { role: data.role, userId: data.userId };
    updateVisibility();
    goTo(data.role === 'MEDICO' ? 'consultas' : 'dashboard');
  }, false);
  $('#presentation-login')?.addEventListener('click', () => {
    state.session = { role: 'APRESENTACAO', userId: null };
    updateVisibility();
    goTo('dashboard');
  });
  ...
  bindForm('#patient-form', async form => { const d = formData(form); const nomePlano = (d.planoNome || '').trim(); await apiRequest(API.pacientes, { method: 'POST', body: JSON.stringify({ nome: d.nome, idade: numberOrNull(d.idade), cpf: d.cpf, telefone: d.telefone, email: d.email || null, planoSaude: { nome: nomePlano || 'Não tenho', numeroCarnetizacao: d.planoNumero || null, ativo: temPlano(nomePlano) } }) }); form.reset(); showAlert('Paciente cadastrado.'); });
  bindForm('#doctor-form', async form => { await apiRequest(API.medicos, { method: 'POST', body: JSON.stringify(formData(form)) }); form.reset(); showAlert('Médico cadastrado.'); });
...
function renderFila(items) { return items.length ? items.map((f, i) => `<article class="list-item searchable"><header><div><strong>#${i + 1} ${escapeHtml(f.pacienteNome || 'Paciente')}</strong><small>${escapeHtml(f.medicoNome || 'Médico')} • ${formatDate(f.dataDesejada || f.dataHora)}</small></div>${badge('Aguardando', 'warn')}</header></article>`).join('') : emptyState('Lista de espera vazia.'); }
...
function getLoggedUser() { if (!state.session) return null; const source = state.session.role === 'MEDICO' ? state.medicos : state.pacientes; return source.find(u => String(u.id) === String(state.session.userId)); }
